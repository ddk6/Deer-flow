...
#这个类用来存储config.yaml中的配置  其实就是将动态的存成静态的 方便后续获取配置  毕竟config.yaml是动态的 每次启动都会重新读取
#这个和那个modelconfig类存储的是不一样的 
...
#这是真正获取config.yaml的函数
# 实际获取的是AppConfig存储的静态配置 我的理解有问题吗？
#   
